...
//===============================================
Level4::Level4() :
	LevelBase(), Button("4", sf::Vector2f(WINDOW_WIDTH / 4 + 17.f, WINDOW_HEIGHT * 0.87 - 4.f), 30)
{
	m_sprite.setPosition(sf::Vector2f(WINDOW_WIDTH / 4, WINDOW_HEIGHT * 0.87));

	m_levelBehavior = SCORE;
	m_name = "Level 4";
	m_levelText = sf::Text("4", Resources::instance().getArielFont(), 35);
	m_levelText.setPosition(sf::Vector2f(WINDOW_WIDTH / 4 - 5, WINDOW_HEIGHT * 0.87 - 20));
	//std::srand(static_cast<unsigned int>(std::time(nullptr)));

	m_backround.setTexture(Resources::instance().getBackroundTexture3());
	...
	for (int i = 0; i < 2; i++)
		m_SmartObjects.push_back(std::make_unique<Shark>(Resources::instance().getSpriteShit(),
			sf::Vector2f(positionRand()), SHARK_SIZE,
			std::bind(&PlayerFish::getLocation, m_player.get()/*, std::placeholders::_1*/), 40.f));
...
std::shared_ptr<LevelBase> Level4::copyMy()
{
	return std::make_shared<Level4>();
}

void Level4::showContentButton(GameController& game, GameBoard& board)
{
	if (m_buttonOn)
	{
